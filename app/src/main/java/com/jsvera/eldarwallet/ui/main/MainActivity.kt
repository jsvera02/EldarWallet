...
import com.jsvera.eldarwallet.ui.scanQr.ScanQrFragment
...
/*            adapterToolbar.addFragment(AddCardActivity(), "AddCardFragment")*/
    ...
    private fun matchFragId(fragId: Int) {
        when (fragId) {
            ...
            R.id.itemCard -> {
                binding.pagerToolbar.setCurrentItem(3, false)
            }
        }
    }
    ...
    private fun matchFragIdReselect(fragId: Int) {
            when (fragId) {
                ...
                R.id.itemCard -> {
                    (binding.pagerToolbar.adapter as? ToolbarPagerAdapter)?.scrollToTop(3)
                }
            }
        }
    }
